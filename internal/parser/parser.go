...
func parseRoot(body hcl.Body) (*entities.Definition, error) {
	rootContent, diags := body.Content(schema.RootSchema)
	if diags.HasErrors() {
		return nil, diags
	}
...
func parseSection(sectionBlock *hcl.Block, level int) (*entities.Section, error) {
	sectionContent, diags := sectionBlock.Body.Content(schema.SectionSchema)
	if diags.HasErrors() {
		log.Print("SECTION")
...
func parseVariable(variableBlock *hcl.Block) (*entities.Variable, error) {
	...
	variableContent, diags := variableBlock.Body.Content(schema.VariableSchema)
	if diags.HasErrors() {
		return nil, diags
	}
...
func parseAttribute(attrBlock *hcl.Block, level int) (*entities.Attribute, error) {
	...
	attrContent, diags := attrBlock.Body.Content(schema.AttributeSchema)
	if diags.HasErrors() {
		return nil, diags
	}
